...
export default function DjSection({ isAdmin }: DjSectionProps) {
  ...
  return (
    <section id="dj" className="scroll-mt-20 py-16">
      ...
      {/* Two-column layout: content left, Lars photo right */}
      <div className="mb-10 grid gap-6 md:grid-cols-[1fr,auto]">
        {/* Left column: bio, Spotify, studio photo, tracks */}
        <div className="space-y-6">
          ...
          {/* Photo gallery (studio + uploaded) */}
          {(GALLERY_PHOTOS.length > 0 || photos.length > 0) && (
            <div className="grid grid-cols-2 gap-3 sm:grid-cols-3">
              {[...GALLERY_PHOTOS, ...photos].map((url, i) => (
                <button
                  key={i}
        ...
        {/* Right column: Lars Vinter portrait */}
        <div className="flex justify-center md:w-72 lg:w-80">
          <button
            onClick={() => setSelectedPhoto("/dj/lars-vinter.png")}
            className="group relative w-full overflow-hidden rounded-xl border border-[var(--border)]"
          >
            <Image
